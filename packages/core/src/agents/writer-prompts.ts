...
function buildChapterTitleRules(): string {
  return `## 章节标题规则

- 标题必须贴合本章真正落地的事件、对象、结果或反转，不能只写空泛动作
- 禁止连续复用最近章节的标题句式；如果最近几章已经出现"先把/先别/先让/谁把/那只/这句"等固定开头，本章必须主动换句型
- 禁止标题首2-3字与最近两章重复；若重复，必须重写
- 优先使用能体现"新信息/新对象/新后果"的标题，不要把不同章节写成可互换模板
- 如果本章并未直接堵到某个人，就不要把标题写成"先把某个人按住"这类过度承诺`;
}
